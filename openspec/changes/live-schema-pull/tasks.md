...
- [x] 1.1 Add a `pull` dispatch branch in `Program.Main` mirroring the `fmt`
      branch (Program.cs:28), delegating to `SchemaPull.Run(args)`.
- [x] 1.2 Parse `pull` flags order-independently: `--cluster <uri>` (required),
...
- [x] 1.3 `SchemaPull.cs`: `HttpClient` POST to `<cluster>/v1/rest/mgmt` with body
...
- [x] 1.4 Add source-gen `System.Text.Json` contexts for the Kusto REST response
...
- [x] 2.4 Confirm a query calling a pulled user-defined function no longer emits
      KQL101 when linted with the generated `--schema` file.

## 3. Optional table sizes (`--with-sizes`) — separable

- [x] 3.1 Fetch table sizes via `.show tables details` (one call, all tables;
...
- [x] 3.2 Compute `factor = max(1, round(size / baseline))`, `baseline` = median
...
- [x] 3.3 Confirm the generated file drives `TableSizeEnricher` unchanged: a big
      table scales KQL003/008 as expected via `lint --table-sizes <file>`.

## 4. Verification

- [x] 4.1 Commit a captured `.show database schema as json` response as a test
...
- [x] 4.2 Self-check (assert-based, in `test/run-tests.sh` style): fixture →
...
- [x] 4.3 Missing token and non-2xx responses exit non-zero with a clear message
      and never print the token.
- [x] 4.4 `dotnet publish -c Release -r linux-x64` succeeds (NativeAOT) with no
      new dependency and no reflection-JSON warnings — proves the single binary
      still builds.
- [x] 4.5 Update README: `pull` subcommand, auth (bearer token via
